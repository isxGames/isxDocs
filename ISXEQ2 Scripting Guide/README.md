...
- **4.26** (2026-08-13) -- Internal efficiency/reliability only
  - Reliability (no scripter-facing surface): `${Actor[id,#]}` is now much more efficient, and a mitigation was added for the Intel Jump Conditional Code (JCC) erratum
  - No KB body changes. No new TLOs, no new datatypes, no new members, methods, commands, events, or removed APIs

- **4.25** (2026-08-08) -- `item` slot API completed its 1-based alignment (behavioral update to existing members)
  - Updated the `item` members `Slot` and `Bag` to document their new **1-based** return values (`Slot` = 1 for the first slot within a bag; `Bag` = 1 for the first bag). `Bag` still returns `-1` for "not in a bag" (equipped, top-level inventory/bank, cursor); `Slot` passes any negative value straight through so the `-1` sentinel is never confused with a real index -- updated in the item Members - Location table of `03_API_Reference.md` and the item Location block of `ISXEQ2_QuickReference.md`
  - Updated the remaining slot-indexed item members/methods to 1-based: `NextSlotOpen` (1 = first open slot; NULL when none -- check `(exists)`), `ItemInSlot[#]` (`ItemInSlot[1]` = first slot; below 1 matches nothing), and the FIRST argument of `Move` (destination slot within the target container; `-1` still means auto-slot; a value below 1 is rejected; the second ContainerID argument and optional Quantity are unchanged) -- updated in the item Container Properties table + Methods table of `03_API_Reference.md` and the item Container/Methods blocks of `ISXEQ2_QuickReference.md`
  - Clarified that `IsSlotOpen[#]` was ALREADY 1-based (unchanged) and that the `Index` member stays **0-based** (the raw inventory array position used by move/equip commands, not a slot number) -- annotated in both files
  - The item members `IsAutoConsumeable` and `CanBeRedeemed`, and the `iteminfo` member `NoExchange`, were bug-fixed in this build; all three were already documented and correct, so no body change was needed
  - No new TLOs, no new datatypes, no new members or methods, no new commands, no removed APIs, no new events

- **4.24** (2026-08-07) -- New `guildbankitem` datatype, guild-bank access, item bag-move methods, and long-item-name support
  - Added new datatype `guildbankitem` (an item in the guild/coalition bank) with members `Page` (int, 1-based 1-4), `Slot` (int, 1-based 1-80), `LinkID` (uint), `ExamineID` (uint), `Quantity` (int), `IsCoalition` (bool), `CanDeposit` (bool), `IsItemInfoAvailable` (bool), `ToItemInfo` (`iteminfo`) and method `MoveToInventory[DestContainerID, DestSlot, Quantity]` (withdraw into inventory; DestSlot is 1-based; Quantity optional; guild vs. coalition auto-selected via `IsCoalition`), plus an async-info note -- added as a new `### guildbankitem` section in `03_API_Reference.md` (also added to the Item DataTypes category index) and a new `#### guildbankitem` block in `ISXEQ2_QuickReference.md`
  - Documented a new `character` member `GuildBank`: `${Me.GuildBank}` (int, number of items currently in the guild bank) and the indexed `GuildBank[#]` / `GuildBank[name]` (`guildbankitem`, `#` is 1-based, 1 to `${Me.GuildBank}`), all returning the integer `0` (not NULL) if the guild bank has not been opened this session -- added to the character Members - Inventory table of `03_API_Reference.md` and the character Inventory block of `ISXEQ2_QuickReference.md`
  - Documented two new `item` methods: `MoveToBag` (two forms -- `[Region, Bag#, Quantity]` into a 1-based bag of Inventory/Bank/SharedBank, or `[GuildBank, Page#, Slot#, Quantity]` into a 1-based guild-bank page 1-4 and slot 1-80) and `MoveAuto[Region, Quantity]` (auto-deposit to a region's auto pad; not applicable to the guild bank). Quantity is optional and defaults to 0 (whole stack); `Shared` is accepted as an alias for SharedBank -- added to the item Methods table of `03_API_Reference.md` and the item Methods list of `ISXEQ2_QuickReference.md`
  - Documented two new `item` members for very-long-named items: `IsNameTruncated` (bool, TRUE only while the name being read right now is still shortened) and `FullName` (string, the complete name, falling back to `.Name` so it never returns empty), plus updated the `Name` member description to note that ISXEQ2 now pulls the complete name from examine info automatically on read (a game-side trimming limit affects only inventory/equipment items) -- added to the item Members - Identity table of `03_API_Reference.md` and the item Identity block of `ISXEQ2_QuickReference.md`
  - Reliability (no scripter-facing surface): fixed a rare hang when reading `eq2icon.IsRecipeInfoAvailable` on a recipe icon whose details had not yet arrived from the server
  - No new TLOs, no new commands, no removed APIs, no new events

- **4.23** (2026-07-28) -- Quest-target/starred actor detection, XP toggles, seller name, and command updates
  - Documented four new `actor` members: `OnJumpingMount` (bool, TRUE on a jumping/leaper mount), `Starred` (bool, TRUE if the actor shows the white AA-star), `QuestTarget` (bool, TRUE if the actor shows the quest feather/quill icon), and `QuestTargetType` (string, WHICH kind of quest update: `None`, `KillTarget`/`BaubleTarget`/`GenericTarget` for a quest in YOUR journal, or `AllyKillTarget`/`AllyBaubleTarget`/`AllyGenericTarget` for a quest in a group member's journal) -- added to the actor Members - State Checks table of `03_API_Reference.md` (with a `QuestTarget` QueryActors example) and the actor state-checks block of `ISXEQ2_QuickReference.md`
  - Documented two new `character` members `CombatXPEnabled` (bool) and `QuestXPEnabled` (bool), and annotated the existing `CombatExpEnabled` as a retained alias for `CombatXPEnabled` -- added to the character Members - Flags table of `03_API_Reference.md` and the character Flags block of `ISXEQ2_QuickReference.md`
  - Documented two new `character` methods `ToggleCombatXP` and `ToggleQuestXP`, and updated `ResetZoneTimer` to document its new optional `"all"` argument (resets every zone reuse timer at once, like "Reset All"; the GUI may not visually update) -- added/updated in the character Methods table of `03_API_Reference.md` and the character Methods list of `ISXEQ2_QuickReference.md`
  - Documented a new `consignment` member `SellerName` (string, the character name of the seller who listed a broker item) -- added to the consignment Members table of `03_API_Reference.md` and the consignment block of `ISXEQ2_QuickReference.md`
  - Documented the `where` command's new `starred` and `questtarget` keywords (list just the starred / quest-target actors), and that every `where` listing now marks each starred actor with a bright-yellow `*` and each quest-target actor with a green `/` before the name -- updated the `#### Where` command section (Syntax, Parameters, Examples, Notes) in `ISXEQ2_QuickReference.md`
  - Documented the `EQ2Loc` command updates: `List` now shows a Distance column, `List dist` sorts by distance, `List <X> <Y> <Z>` / `Find <X> <Y> <Z>` shows the 5 nearest saved locations to a coordinate, saved locations now live in an auto-created/migrated `eq2locations` folder (multi-file read; only `isxeq2locations.xml` is written), and adding/removing a location live-syncs across multiple running instances -- updated the `#### eq2loc` command section (Syntax, Parameters, Examples, Notes) in `ISXEQ2_QuickReference.md`
  - The `actor.IsNPC` int->bool change from this build was already documented at 4.18 (no change needed here)
  - Reliability (no scripter-facing surface): recovery from a stalled/contended update check when launching multiple EQ2 instances at once, and crash-hardening of the ContainerWindow, RewardWindow, ChoiceWindow, ReplyDialog and ExamineItemWindow TLOs when read from inside their matching "...WindowAppeared" event
  - No new TLOs, no removed APIs, no new events

- **4.22** (2026-07-28) -- Documented the `Math` TLO and the full `math` object type in the Quick Reference
  - Added a **Math** row to the "Inner Space TLOs (Base / Game-Agnostic)" table and a new `### Math` subsection to `ISXEQ2_QuickReference.md`, with a complete member table: `Calc[formula]` (float64ptr), `Calc64[formula]` (int64ptr), `Abs[formula]` (float), `Sqrt[formula]` (float), `Rand[formula]` (int, 0 to # - 1), `Sin`/`Cos`/`Tan[formula]` (float, degrees), `Asin`/`Acos`/`Atan[formula]` (float, degrees), `Atan[formula,formula]` (float, atan2 in degrees), `DistancePointLine[x,y,ax,ay,bx,by]` (float), `Hex[#]` (string), `Dec[hex]` (int), `Not[#]` (int)
  - Documented all three argument forms of `Distance`: `Distance[x1,x2]` is a **1-dimensional** distance, `Distance[x1,y1,x2,y2]` is 2-dimensional, `Distance[x1,y1,z1,x2,y2,z2]` is 3-dimensional. Added an explicit note that the two-argument form is valid and is the idiomatic single-axis test, with a vertical-tolerance example, because a 2-argument call is otherwise easily misread as an incomplete 2-D call
  - Added a note distinguishing the `math` object type from the actor `Distance` / `Distance2D` members, which measure from the player and are unrelated
  - The Quick Reference previously had no `math` coverage at all, so the accepted argument forms were not answerable from it -- the entry in `01b_LavishScript_Reference.md` was already correct and is unchanged
  - No new ISXEQ2 TLOs, datatypes, members, methods, commands, events, or removed APIs -- this documents a pre-existing base Inner Space object type (game-agnostic, not ISXEQ2-specific) that was undocumented in the Quick Reference

- **4.21** (2026-07-27) — Documented the HUD / HUDSet command syntax, flags and default font size
  - Added a "Syntax" block to the HUD section of `01b_LavishScript_Reference.md` giving the full `HUD` and `HUDSet` command forms verbatim, plus a flag table for `HUDSet` (`-s` size, `-c` color, `-l` position, `-t` text, `-f` filter) and a create-then-style example
  - Documented that `HUD -add` accepts only name, `X,Y` and text — it has NO size or color parameter, so appearance must be applied afterwards with `HUDSet`
  - Documented that the default HUD font size is 12 (reported as `[Size:12]` by `HUD -list`; the command reference does not state it as a default), which is what makes `-s 24` double size
  - Documented that `HUDSet *` targets EVERY HUD element in the session, including those owned by other running scripts, so a script should name its own elements instead
  - Documented that HUD elements are positioned by absolute `X,Y`, so font size and line spacing are independent — scaling the text requires scaling the Y spacing by the same factor or lines overlap
  - The section previously consisted of three wiki links with no syntax, no flags and no defaults, so none of this was answerable from the KB
  - No new ISXEQ2 TLOs, datatypes, members, methods, commands, events, or removed APIs — this documents pre-existing base Inner Space commands (game-agnostic, not ISXEQ2-specific) that were under-documented

- **4.20** (2026-07-27) — Documented the LavishGUI 1 `SetFocus` method and `Focus` member
  - Documented `SetFocus` as an element method that gives keyboard focus to any element that can accept it — not only windows. Added a "SetFocus - Give an element keyboard focus" subsection to "Modifying UI Elements from Script" in `08_LavishGUI1_UI_Guide.md`, covering the two things that make it fail in practice: the element must be shown FIRST (a hidden element cannot take focus), and focus is granted only "if it can accept it"
  - Documented the companion `Focus` member (bool, TRUE while the element holds keyboard focus) in the "Reading UI Element State" section of `08_LavishGUI1_UI_Guide.md`, with the check-after-SetFocus idiom that distinguishes "focus took" from "focus was refused"
  - Added a cross-reference from "TextBoxes (Text Input)" to the new subsection, since putting the caret in a box on open is the case that sends people looking
  - The guide previously showed `This:SetFocus` only as a bare `<OnLeftClick>` line on a Window (`08_LavishGUI1_UI_Guide.md`) with no explanation, and never mentioned `Focus` at all — neither was discoverable as an element-level capability
  - No new ISXEQ2 TLOs, datatypes, members, methods, commands, events, or removed APIs — this documents pre-existing LavishGUI 1 functionality (game-agnostic, not ISXEQ2-specific) that was under-documented

- **4.19** (2026-07-26) — Documented `string.Find` as case-insensitive
  - Documented that the LavishScript `string` member `Find[text]` performs a CASE-INSENSITIVE substring search (returns the 1-based position of the substring, or NULL if not found, regardless of case) and that there is NO case-sensitive `FindCS` companion. The docs were previously silent on case behavior. Updated the `Find[text]` row of the string-member table in `01b_LavishScript_Reference.md` (§3.1 Text), the `.Find[substring]` row of the string-members table in `01_LavishScript_Fundamentals.md`, and added a note to the "Case Sensitivity" section of `ISXEQ2_QuickReference.md` (where `.Find` previously appeared only in usage examples, with no member description)
  - No new TLOs, no new datatypes, no new members or methods, no new commands, no removed APIs, no new events — this documents the existing case behavior of an existing member

- **4.18** (2026-07-23) — `actor.IsNPC` changed from int to bool, plus a general bool-member query idiom note
  - Changed the `actor` member `IsNPC` from `int` (`1`/`0`) to `bool` (`TRUE`/`FALSE`) — it still means "the actor is an NPC or NamedNPC" and is still a cheaper `QueryActors` filter than `(Type =- "NPC" || Type =- "NamedNPC")`. Updated its type and description in `03_API_Reference.md` (actor Members - State Checks table) and `ISXEQ2_QuickReference.md` (actor state-checks block), and updated every `IsNPC == 1` example to the bare-name form `IsNPC` in `03_API_Reference.md`, `04_Core_Concepts.md`, and `00_MASTER_GUIDE.md`, plus the `actor.IsNPC` prose in the `04_Core_Concepts.md` "QueryActors Sort Modes" paragraph
  - Documented the QueryActors bool-member query idiom (applies to ANY bool member, not just `IsNPC`): a boolean member is tested by BARE NAME for TRUE and a leading `!` for FALSE (`IsNPC` / `!IsNPC`) and must NOT be written as `== 1`, `== TRUE`, or `== FALSE` (the query evaluator does not equate a bool with `1`/`TRUE`/`FALSE`, so those forms match nothing). Added as a "Boolean members in a query" note after the query-operator table in `04_Core_Concepts.md` (with both `IsNPC && ...` and `!IsNPC && ...` worked examples) and in the query-operators comment block of the QueryActors section in `ISXEQ2_QuickReference.md`
  - No new TLOs, no new datatypes, no new members or methods, no new commands, no removed APIs, no new events — this changes the type of one existing member and documents an existing query idiom

- **4.17** (2026-07-23) — Documented the base Inner Space `Input` key/button-state read side
  - Documented reading current key/button state via `${Input.Button[<name>].Pressed}` (bool, TRUE while the key/button is held) — a base Inner Space / LavishScript feature (game-agnostic, not ISXEQ2-specific), and the READ complement to the write-only `Press` command and the `Keyboard`/`Mouse` emulation objects. Canonical example: `${Input.Button[alt].Pressed}`
  - Fleshed out the `input` object type in `01b_LavishScript_Reference.md` (§3.18 Inner Space Input): the `input` row now lists its members (`Button[name]`/`Button[#]` -> `button`, `Axis`, `DPad`, `Bind`, `BindsEnabled`) and methods (`Bind`, `EnableBinds`, `DisableBinds`, `GetButtonIterator`); the `button` row now lists its members `Name` (string), `Device` (string), `ID` (uint), `Pressed` (bool) and methods `Press`/`Hold`/`Release`; added a prose note with the ALT copy-paste example. Also updated the `Input` TLO row in §4.2 to point at the fleshed-out type with the `Pressed` usage
  - Added a "Reading Current Key/Button State (Input.Button)" subsection to the Input Emulation section of `01_LavishScript_Fundamentals.md` with the ALT example
  - Added an "Inner Space TLOs (Base / Game-Agnostic)" subsection to the Top-Level Objects section of `ISXEQ2_QuickReference.md` (Input/Keyboard/Mouse table plus the `Input.Button[name].Pressed` read-state explanation, ALT example, and the `button` object member list)
  - No new ISXEQ2 TLOs, datatypes, members, methods, commands, events, or removed APIs — this documents a pre-existing base Inner Space feature that was under-documented

- **4.16** (2026-07-19) — Cross-file sync and a newly documented behavior quirk
  - Synced the six `character` combat-stat members already documented in `03_API_Reference.md` as of 4.12 into `ISXEQ2_QuickReference.md`, all `float` percentages with paired value/cap members (the live value is clamped to <= its `*Max`): `CritBonus` and `CritBonusMax` (Crit Bonus stat and cap), `WeaponDamage` and `WeaponDamageMax` (Weapon Damage (Bonus) stat and cap), `Fervor` and `FervorMax` (Fervor stat and cap) — added as a new `*Combat Stats:*` sub-block in the `ISXEQ2_QuickReference.md` character Stats block, after `BaseIntelligence`. Not a new API; this only brings the QuickReference in line with the API Reference
  - Documented a previously undocumented behavior of the `eq2` datatype's `QueryActors` method: the optional `sortmode` argument is silently ignored when the query expression is empty (an empty query is handled by a separate internal code path that takes no sort parameter and always sorts), so `EQ2:QueryActors[Actors, "", "NoSort"]` returns a sorted index anyway — the sort mode only takes effect with a non-empty query. Added as a "Note (empty query)" callout after the "QueryActors Sort Modes" table in `03_API_Reference.md` and appended to the `QueryActors` method entry in the `ISXEQ2_QuickReference.md` eq2 block. Not a new API; this documents existing behavior
  - No new TLOs, no new datatypes, no new members or methods, no new commands, no removed APIs, no new events
