...
- **v4.0 (2026-04-28)**
  ...
  - Applied 18 HIGH-severity in-place fixes to `01` per `reports/ISXEVE_KnowledgeBase_Report_04.27.2026.md`: reframed the tilde-escape rule from "best practice" to REQUIRED for re-parsed output; documented the `(parent)` keyword cast as distinct from the named-type `(typeName)` cast; introduced the `variable(scope)` qualifier (script / global / globalkeep) in Declaring Variables; named `${Return}` as a TLO and linked the wiki; added Inner-Space-Kernel provenance labels to the Audio System and Web Requests sections; corrected the misleading `wait -s 0.001` example to clarify frame-quantization; merged "Methods as Commands" with "Method Return Values" into a single "Methods in Data Sequences vs As Commands" subsection; renamed the atexit example global from `Script` (which would shadow the built-in `Script` TLO) to `App` / `MyApp`; combined the three `wait` forms (deciseconds, `-s` seconds, conditional) into one section; expanded the data-types table to all eight numeric types (int / uint / int64 / byte / float / float64 / bool / string) with a Type Conversion subsection covering the Bool / Float / Int / String / Enum TLOs; expanded Type Inspection to all five information commands (Commands / LSType / LSVersion / Scripts / TopLevelObject); softened the "braces must be on their own line" rule with the JSON-literal exception; updated all wiki URLs in Additional Resources to clickable canonical-form `https://...index.php/...` markdown links
  - Locked the scope of both `01` and `01b` to LavishScript-core, Inner-Space-core, and bundled first-party subsystem packages (LavishGUI, LavishGUI 2, LavishNav, LavishSettings, LavishMachine, LSModule:*) -- ZERO game-specific or extension-specific content. No `${Me}` / `${MyShip}` / `${Entity}` / `${EVE}` / `${EQ2}` / `ISXEVE` / `ISXEQ2` references in either file. Game-extension TLOs and datatypes belong in their respective extension-specific guide files. Verified by grep across both files
  - Established the skip-block convention for the split: `01` retains `<!-- CLAUDE_SKIP_START -->` / `<!-- CLAUDE_SKIP_END -->` markers around motivational / pedagogical narrative ("Running Your First Script", LERN attribution); `01b` is pure reference with NO skip-blocks (the agent reads all of it for lookup-style queries)
  - Cross-reference sweep across nine KB-tree dependents: `README.md` (Documentation Structure now lists `01` as `2.` and `01b` as `2b.`; By-Task table gains a "LavishScript / Inner Space command/datatype/TLO lookup" row); `00_MASTER_GUIDE.md` (fixed broken `01_LavishScript_Fundamentals.md#commands` anchor → routed to `01b §2-commands`; By-Learning-Level table gains a lookup-row); `+How To Use This Guide with Claude Code+.md` (two phrases updated to mention the `01b` sibling); `ISXEVE_QuickReference.md` (Cross-References footer gains an `01b` row); `02_Quick_Start_Guide.md` (prerequisite pointer mentions both files); `13_JSON_Guide.md` (broken anchor `#collections-and-lists-index` → `#collections-and-lists` + container-reference pointer to `01b §3.5`); `14_LavishMachine_Guide.md` (both Choose-the-right-API call-out boxes for Audio Tasks and Web Request Tasks now point at both `01` for tutorial intro and `01b` for full type/method reference); `Claude AI Commands (optional)/isxeve.md` and `Claude AI Commands (optional)/ISXEVE-Expert.md` (in-tree masters of the Claude Code coordinator and worker agent both gain `01b` entries in their guide-files and large-documentation-files lists). Two files (`05_Patterns_And_Best_Practices.md`, `16_Mining_And_Hauling.md`) verified as no-action -- their grep hits were false positives ("Timing Fundamentals" / "Mining Fundamentals" internal section names, not pointers to file 01)
  - Resolved all 82 audit findings catalogued in `reports/ISXEVE_KnowledgeBase_Report_04.27.2026.md` (18 HIGH + 40 MEDIUM + 24 LOW). Phase 1 (`01b` written from scratch) resolved 26 directly; Phase 2 (`01` rewrite) resolved another 28 plus the 10 partially-resolved findings; Phase 3 (LOW-severity polish + cross-reference sweep) resolved the remaining 18. Surfaced 16 wiki-verification gaps (entries listed on canonical wiki index pages or templates but lacking dedicated `Command:` / `ObjectType:` / `TLO:` detail pages -- e.g., `Diagnostics`, `RelayTargets`, `DotScript`, `jsoniterator`, `anonevent`, `taskpulseargs`, `elmactaskstate`, `xmlnode`, `xmlreader`, `distributedscope`, `distributedvalue`, `midioutdevice`, `dataset`, `Context` TLO, `LGUI2` TLO, `LMAC` TLO); flagged inline in `01b` and tabulated in the audit report so future updates can refine to dedicated-page citations if those wiki pages get written
