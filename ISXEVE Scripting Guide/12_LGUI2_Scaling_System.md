...
The base custom title bar pattern (the dockpanel structure, close/shade button definitions, event-forwarding handlers, and the script-side `Shutdown` atom invoked via `QueueCommand`) is documented in [10_LavishGUI2_UI_Guide.md](10_LavishGUI2_UI_Guide.md#window). This section focuses on the scaling-specific aspects: how to size the title bar elements so they scale correctly with the LGUI2 Scaling System, and why a custom title bar is required in the first place.
