...
10. [Complete Working Examples (Fleet Coordination)](#complete-working-examples-fleet-coordination)
11. [Common Problems (Fleet)](#common-problems)

### Relay System and IPC
12. [Relay System and Inter-Process Communication](#relay-system-and-inter-process-communication-ipc)
13. [LavishScript Relay Basics](#lavishscript-relay-basics)
14. [Event System Foundation](#event-system-foundation)
15. [Basic Relay Patterns](#basic-relay-patterns)
16. [UplinkManager System](#uplinkmanager-system)
17. [Fleet Coordination Patterns](#fleet-coordination-patterns)
18. [IRC Bridge Integration](#irc-bridge-integration)
19. [Uplink Networking](#uplink-networking)
20. [Performance Considerations (Relay)](#performance-considerations)
21. [Best Practices (Relay)](#best-practices-relay)
22. [Complete Working Examples (Relay System)](#complete-working-examples-relay-system)

### Configuration Management
23. [Configuration and Settings Management](#configuration-and-settings-management)
24. [LavishSettings Foundation](#lavishsettings-foundation)
25. [EVEBot Configuration Architecture](#evebot-configuration-architecture)
26. [Tehbot Configuration Architecture](#tehbot-configuration-architecture)
27. [Configuration Patterns](#configuration-patterns)
28. [XML File Structure](#xml-file-structure)
29. [Config Macros and Helpers](#config-macros-and-helpers)
30. [UI Integration](#ui-integration)
31. [Per-Character Settings](#per-character-settings)
32. [Config Migration](#config-migration)
33. [Fleet Config Synchronization](#fleet-config-synchronization)
34. [isxSQLite Configuration Storage](#isxsqlite-configuration-storage)
35. [Best Practices (Configuration)](#best-practices-configuration)
36. [Complete Examples (Config)](#complete-examples)
...
A minimal Fleet-Commander / DPS-Ship split for primary-target calling was shown earlier as a sketch. The full production-grade implementation — with master/slave role detection, primary and secondary target broadcasting, slave-side `OnPrimaryTarget` / `OnSecondaryTarget` atoms, lock-and-fire logic, fleet warp, anchor requests, EWAR assignment, and complete Initialize/Shutdown lifecycle — is presented as the canonical example in [Example 1: Complete Fleet Combat Coordination](#example-1-complete-fleet-combat-coordination) below. Refer to that section for the single, authoritative `obj_FleetCombat` object and use it as the starting point for any FC-driven combat fleet.
...
The full Orca service pattern — including miner-side survey-scan and shield-boost requests, Orca-side request handlers, event registration, and shield-transporter activation — is integrated into the complete `obj_MiningFleet` implementation shown later in this guide under [Example 2: Mining Fleet with Orca Support](#example-2-mining-fleet-with-orca-support). See that section for the complete canonical Orca-centric coordination implementation.
...
A full config system implementation is already covered earlier in this guide. Rather than duplicate it here with renamed `obj_MyBot_*` classes, see the canonical references:

- [Tehbot Configuration Architecture](#tehbot-configuration-architecture) — `obj_Configuration_Manager` singleton, `obj_Configuration_Base` base class, and `obj_Configuration_Common` derived class, plus the `Setting()` macro that generates member/method pairs in one line
- [EVEBot Configuration Architecture](#evebot-configuration-architecture) — The alternative `obj_Configuration_BaseConfig` pattern with a main `obj_Configuration` object that holds sub-config objects

**To build a domain-specific config class (Combat, Mining, Hauling, etc.) for your own bot:**

1. Inherit from `obj_Configuration_Base` and call `This[parent]:Initialize["YourSetName"]` — see [Derived Configuration Classes](#derived-configuration-classes)
2. Override `Set_Default_Values` to seed the new settings with `This.ConfigRef:AddSetting[...]`
3. Expose each setting as a member/method pair — manually per [Pattern 1: Member/Method Pair](#pattern-1-membermethod-pair), or concisely via the `Setting()` macro
4. Add range/sanity checks to setter methods per [2. Validate Config Values](#2-validate-config-values)
5. Register the derived config in your main `obj_Configuration` object as a `variable` member
6. For schema evolution across versions see [Config Migration](#config-migration)

Following that recipe, a Combat config with `MinShield`, `MinArmor`, `MinCap`, `OrbitDistance`, and `UseDrones` is a straightforward ~30-line derived class — no new architecture required.
