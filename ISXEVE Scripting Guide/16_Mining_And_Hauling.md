...
The full Orca-detection and ore-delivery pattern — finding the Orca by pilot name, approaching it, opening its fleet hangar, and transferring ore via the modern Inventory API — is consolidated into the canonical `obj_OrcaDelivery` implementation shown later in this guide under [Delivering Ore to Orca](#delivering-ore-to-orca). See that section for the complete `FindOrca` and `DeliverToOrca` functions wrapped in a proper objectdef.
...
Complete hauler that supports a mining fleet by collecting full jetcans from miners and ferrying ore back to station.

Rather than duplicating ~280 lines of nearly-identical state machine code, this example is presented as a **delta** over [Example 1: Simple Station-to-Station Hauler](#example-1-simple-station-to-station-hauler). Start from `obj_SimpleStationHauler` and apply the following adaptations:

- **Event-driven pulse**: Attach `This:Pulse` to `EVENT_ONFRAME` in `Initialize()` instead of running a blocking `while TRUE` loop in `Start()`.
- **Fleet event listener**: Register a custom `EVEBot_Miner_Full` event and attach an atom (e.g., `OnMinerFull(fleetMemberID, systemID, canID)`) that queues incoming can IDs into a `variable queue:int64 FullMinerCans`.
- **New states**: Add `COLLECT_CANS`, `RETURN_TO_STATION`, `RETURN_TO_MINING`, `WAIT_AT_SAFE`, and `PODDED` to `DetermineState` / `ExecuteState`, replacing the station-to-station `LOAD` / `TRAVEL_TO_PICKUP` / `TRAVEL_TO_DELIVERY` flow.
- **Cargo threshold**: Trigger `RETURN_TO_STATION` on `${Ship.Cargo.PercentFull} > 90` instead of a fixed `UsedCapacity` comparison.
- **Can collection logic**: Implement `CollectCans()` to peek the queue, warp to the can if `Distance > 150000`, `Ship.Approach` to `LOOT_RANGE`, `CmdStopShip`, then call a `LootCan(canID)` helper that opens the can, batches item IDs via `EVE:MoveItemsTo[itemIDs, MyShip, CargoHold]`, and dequeues.
- **Return to mining**: After unloading, call `Station.Undock` then `Navigator:FlyToBookmark["${MiningSystem}", 0]` to resume support duty.
- **Idle behavior**: When in the mining system with no queued cans, warp to a safespot via `Safespots.WarpTo` (the `WAIT_AT_SAFE` state) rather than sitting stationary.
- **Pod safety**: Detect `${Ship.IsPod}` early in `DetermineState` and force `EmergencyDock`.

These adaptations layer cleanly onto the `obj_SimpleStationHauler` skeleton — the `FLEE` / `EmergencyDock` / `UnloadCargo` helpers carry over unchanged. For event registration and atom patterns, see [Listen for Miner Full Events](#listen-for-miner-full-events) earlier in this chapter.
